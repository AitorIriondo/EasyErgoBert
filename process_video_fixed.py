"""
Fixed 3D Pose Estimation Pipeline with correct MotionBERT integration.

Uses YOLOv8-Pose for 2D detection + MotionBERT pose3d checkpoint for 3D lifting.

Requirements:
- body4d conda environment (has RTX 5090 GPU support with PyTorch 2.9.1+cu128)
- MotionBERT pose3d checkpoint: checkpoint/pose3d/FT_MB_lite_MB_ft_h36m_global_lite/best_epoch.bin
  Download from: https://1drv.ms/f/s!AvAdh0LSjEOlgT67igq_cIoYvO2y?e=bfEc73

Usage:
    # With body4d environment (GPU)
    conda activate body4d
    python process_video_fixed.py --input input/video.mp4 --output output/ --render

    # CPU fallback
    python process_video_fixed.py --input input/video.mp4 --output output/ --device cpu
"""
...
def check_checkpoint():
    """Check if the correct pose3d checkpoint exists."""
    ckpt_path = MOTIONBERT_PATH / "checkpoint" / "pose3d" / "FT_MB_lite_MB_ft_h36m_global_lite" / "best_epoch.bin"
    if ckpt_path.exists():
        size_mb = ckpt_path.stat().st_size / (1024 * 1024)
        print(f"Pose3D checkpoint found: {ckpt_path.name} ({size_mb:.1f} MB)")
        return str(ckpt_path)

    # Check for wrong checkpoint (mesh model)
    mesh_ckpt = MOTIONBERT_PATH / "checkpoint" / "mesh" / "FT_MB_release_MB_ft_pw3d" / "best_epoch.bin"
    if mesh_ckpt.exists():
        print("\nWARNING: Found MESH checkpoint but NOT pose3d checkpoint!")
        print("The mesh checkpoint is for body mesh reconstruction, NOT 3D pose estimation.")
...
class MotionBERTPose3D:
    """
    MotionBERT 3D pose lifter using the correct pose3d model.

    IMPORTANT: This requires the pose3d checkpoint, NOT the mesh checkpoint!
    - pose3d checkpoint: FT_MB_lite_MB_ft_h36m_global_lite/best_epoch.bin
    - mesh checkpoint (WRONG): FT_MB_release_MB_ft_pw3d/best_epoch.bin
    """
    ...
    def __init__(self, device: str = 'cuda:0'):
        ...
        # Config path
        config_path = MOTIONBERT_PATH / "configs" / "pose3d" / "MB_ft_h36m_global_lite.yaml"
    ...
    def _load_model(self, checkpoint_path: str, config_path: Path):
        ...
        try:
            ...
            # Create model - IMPORTANT: dim_in=3 for (x, y, conf) input
            # When no_conf=True, we strip confidence before model, so dim_in=2
    ...
    def _flip_data(self, data):
        ...
        Left-right swap indices for H36M:
        - Legs: (1,4), (2,5), (3,6) - RHip/LHip, RKnee/LKnee, RFoot/LFoot
        - Arms: (14,11), (15,12), (16,13) - RShoulder/LShoulder, etc.
        """
        flipped = data.clone()
    ...
    def lift(self, keypoints_2d: np.ndarray, scores: np.ndarray,
        ...
        # Normalize keypoints to [-1, 1] range centered at image center
        # This is the standard MotionBERT normalization
        ...
        # Combine keypoints with confidence: (T, 17, 3)
        if self.no_conf:
            input_data = kpts
        else:
        ...
        with torch.no_grad():
            for start in range(0, T, stride):
                ...
                # Post-process
                if self.rootrel:
                    pred[:, :, 0, :] = 0  # Root at origin
                else:
                    # Global mode: first frame root Z at 0
                    pred[:, 0, 0, 2] = 0
                ...
                if start == 0:
                    ...
                    # Middle window - keep center portion
                    keep_start = stride // 2
...
class PoseEstimationPipeline:
    """Fixed 3D pose estimation pipeline."""
    ...
    def process_video(self, video_path: str, output_dir: str,
                      person_idx: int = 0, render_video: bool = False) -> Dict:
        ...
        all_keypoints_h36m = []
        all_scores = []
        all_frames = [] if render_video else None
        ...
        if self.lifter.model is not None:
            ...
            # Validate output
            self._validate_3d_output(keypoints_3d)
        else:
        ...
        if render_video and all_frames:
            self._render_video(all_frames, output_dir, video_name,
                             keypoints_h36m, scores, fps)
    ...
    def _validate_3d_output(self, keypoints_3d: np.ndarray):
        """Validate 3D output by checking bone lengths."""
        # H36M bone definitions
        bones = [
            (0, 7, 'Hip-Spine', 0.10, 0.25),      # Expected 10-25cm
            (7, 8, 'Spine-Thorax', 0.15, 0.35),   # Expected 15-35cm
            (8, 9, 'Thorax-Neck', 0.10, 0.25),    # Expected 10-25cm
            (9, 10, 'Neck-Head', 0.15, 0.30),     # Expected 15-30cm
            (0, 1, 'Hip-RHip', 0.05, 0.20),       # Expected 5-20cm
            (1, 2, 'RHip-RKnee', 0.35, 0.55),     # Expected 35-55cm
            (2, 3, 'RKnee-RFoot', 0.35, 0.55),    # Expected 35-55cm
            (8, 11, 'Thorax-LShoulder', 0.10, 0.30),
            (11, 12, 'LShoulder-LElbow', 0.25, 0.40),
            (12, 13, 'LElbow-LWrist', 0.20, 0.35),
        ]

        print("\nValidating 3D output (bone lengths):")
        frame0 = keypoints_3d[0]
        warnings = 0

        for start, end, name, min_len, max_len in bones[:5]:
            length = np.linalg.norm(frame0[end] - frame0[start])
            status = "OK" if min_len <= length <= max_len else "WARNING"
            if status == "WARNING":
                warnings += 1
            print(f"  {name}: {length:.3f}m [{status}]")

        if warnings == 0:
            print("  All bone lengths look reasonable!")
        else:
            print(f"  {warnings} bone(s) outside expected range")
            print("  (This may indicate issues with the 3D lifting)")

    def _render_video(self, frames, output_dir, video_name, keypoints_2d, scores, fps):
        """Render output video with skeleton overlay."""
        ...
        for frame_idx, frame in enumerate(tqdm(frames, desc="Rendering")):
            if frame_idx >= len(keypoints_2d):
                break
            ...
            # Draw skeleton
            for i, j in skeleton:
                if scrs[i] > 0.3 and scrs[j] > 0.3:
                    pt1 = tuple(kpts[i].astype(int))
                    pt2 = tuple(kpts[j].astype(int))
                    cv2.line(frame, pt1, pt2, (0, 255, 0), 2)

            # Draw joints
            for i, (pt, score) in enumerate(zip(kpts, scrs)):
                if score > 0.3:
                    color = (0, 0, 255) if i in [0, 7, 8, 9, 10] else (255, 0, 0)
...
def main():
    parser = argparse.ArgumentParser(description='Fixed 3D Pose Estimation Pipeline')
    ...
    parser.add_argument('--render', '-r', action='store_true', help='Render skeleton video')
    ...
    if not args.input:
        print("Error: --input is required (unless using --check-only)")
        sys.exit(1)
