...
class COCOtoH36MConverter:
    ...
    def convert_batch(self,
                      coco_keypoints_batch: np.ndarray,
                      scores_batch: Optional[np.ndarray] = None) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        """
        Convert a batch of COCO keypoints to H36M format.

        Args:
            coco_keypoints_batch: Array of shape (B, T, 17, 2) or (B, T, 17, 3)
                                  B = batch size, T = temporal length
            scores_batch: Optional confidence scores, shape (B, T, 17)

        Returns:
            Tuple of (h36m_keypoints_batch, h36m_scores_batch)
        """
        batch_size = coco_keypoints_batch.shape[0]
        results_kpts = []
        results_scores = []

        for b in range(batch_size):
            kpts, scores = self.convert(
                coco_keypoints_batch[b],
                scores_batch[b] if scores_batch is not None else None
            )
            results_kpts.append(kpts)
            if scores is not None:
                results_scores.append(scores)

        h36m_kpts = np.stack(results_kpts, axis=0)
        h36m_scores = np.stack(results_scores, axis=0) if results_scores else None
...
def test_converter():
    """Test the COCO to H36M converter."""
    # Create sample COCO keypoints (17 joints, x/y coordinates)
    np.random.seed(42)
    coco_kpts = np.random.rand(17, 2) * 100 + 50  # Random coords in [50, 150]
    coco_scores = np.random.rand(17)  # Random confidence scores
    ...
    print("\nH36M Joint positions:")
    for i, name in enumerate(converter.H36M_JOINT_NAMES):
        print(f"  {i:2d}. {name:12s}: ({h36m_kpts[i, 0]:.2f}, {h36m_kpts[i, 1]:.2f}), "
              f"score: {h36m_scores[i]:.3f}")

    # Test batch conversion
    batch_kpts = np.random.rand(10, 17, 2) * 100 + 50  # 10 frames
    h36m_batch, _ = converter.convert(batch_kpts)
    print(f"\nBatch input shape: {batch_kpts.shape}")
    print(f"Batch output shape: {h36m_batch.shape}")
